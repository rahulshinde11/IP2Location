...
creates missing mount sources as root, which is what previously forced a manual
`sudo chown -R 1000:1000 ...` on the host). After fixing ownership it drops to
...
CHOWN_DIRS = os.getenv("CHOWN_DIRS", "/app/db:/downloads:/backups:/app/logs").split(":")
